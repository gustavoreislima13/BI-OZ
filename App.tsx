...
class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  render() {
    if (this.state.hasError) {
      return <div className="p-8 text-center text-red-600">Algo deu errado. Por favor, recarregue a página.</div>;
    }
...
function App() {
  ...
  const handleDeleteSale = async (id: string) => {
    try {
      ...
      await deleteSale(id);
      await fetchData();
      // Não precisa de alerta para exclusão, a atualização da lista é visual o suficiente
    } catch (error) {
      console.error(error);
        ...
        >
          <div className="h-full flex flex-col w-64">
            ...
            <div className="p-4 border-t border-gray-100">
               ...
               <p className="text-xs text-center text-gray-400 mt-4 whitespace-nowrap">v2.1.0 • Stable</p>
            </div>
          </div>
        </aside>
